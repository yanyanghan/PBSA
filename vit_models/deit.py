...
class VanillaVisionTransformer(VisionTransformer):
    ...
    def forward_features(self, x):
        ...
        for idx, blk in enumerate(self.blocks):
            x, a_ma = blk(x)  # 第idx层encoder的输出为x，a_ma为multi-head的输出
            layer_wise_tokens.append(x)  # 将每一层的输出append
            layer_attention.append(a_ma)  # 将每一层encoder的multi-head输出append

        layer_wise_tokens = [self.norm(x) for x in layer_wise_tokens]  #### 此处，是否需要将attention相同操作～～～～～～～

        return [x[:, 0] for x in layer_wise_tokens], [x for x in layer_wise_tokens], layer_attention  #[a_ma for a_ma in layer_attention]

    def forward(self, x, patches=False, only_last=False):
        list_out, patch_out, patch_a_ma = self.forward_features(x)
        if only_last:
            return self.head(list_out[-1]), patch_a_ma
...
def deit_tiny_patch16_224(pretrained=False, **kwargs):
    model = VanillaVisionTransformer(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)  # depth表示为encoder的层数，num_heads为多头数量
