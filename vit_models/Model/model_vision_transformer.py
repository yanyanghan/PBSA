...
class Block(nn.Module):
    ...
    def forward(self, x):
        A_MA = self.drop_path(self.attn(self.norm1(x)))  # 第idx层encoder的multi-head的输出
        x = x + A_MA  # 后面一部分为一个encoder的multi-head的总输出
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if hybrid_backbone is not None:
            ...
            self.patch_embed = PatchEmbed(
                img_size=224, patch_size=16, in_chans=1024, embed_dim=embed_dim)
    ...
    def forward_features(self, x):
        ...
        for blk in self.blocks:
            x, a_ma = blk(x)  # a_ma为multi-head的输出

        x = self.norm(x)
        return x[:, 0], a_ma

    def forward(self, x):
        x, a_ma = self.forward_features(x)
        x = self.head(x)
        return x, a_ma
